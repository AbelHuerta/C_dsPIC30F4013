...
uint8_t I2C_Read_Data(uint8_t ACK){
    ...
    // Configurar el dispositivo como receptor
    SSPCON2bits.RCEN = 1;
    ...
    // Enviar la señal de ACK o NACK al dispositivo esclavo
    SSPCON2bits.ACKDT = (ACK == 0) ? 1 : 0; // NACK si ACK = 0, ACK si ACK = 1
    SSPCON2bits.ACKEN = 1;
    
    // Esperar a que se complete el envío de la señal de ACK o NACK
